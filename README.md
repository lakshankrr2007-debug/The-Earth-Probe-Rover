...
🔄 Working Principle
The user selects a movement direction in the Bluetooth RC Controller app.
The phone transmits a character such as F, B, L, R, or S.
The Bluetooth module forwards the command to the Arduino.
The Arduino interprets the command.
The motor driver supplies the required direction/current to the motors.
Three motors on each side operate together to move or turn the rover.
Environmental sensors collect measurements.
Sensor readings are transferred to the Flask application.
The browser dashboard periodically retrieves the latest readings and displays them.
⚠️ Power and Safety
